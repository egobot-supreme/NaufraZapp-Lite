...
if (!args[0]) return conn.reply(m.chat, `*『✦』Ingrese el enlace valido ppr favor.*`, m, fake, )
if (!args[0].match(/latestmodapks/gi)) return conn.reply(m.chat, `『✦』El enlace ingresado no es valido.`, m, fake, )

try {
  
let res = await fetch(`https://api.akuari.my.id/downloader/dlmod?link=${args[0]}`)
let json = await res.json()
await conn.reply(m.chat, `『✦』TAMAÑO: ${json.respon.size}\n\n『⏳』Se esta descargando...`, m, fake, )
...
return conn.reply(m.chat, '*『✦』Ocurrio un error inesperado.*', m, fake, )}
